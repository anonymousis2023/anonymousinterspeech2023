...
We propose UnitSpeech, a speaker-adaptive speech synthesis method that fine-tunes the pre-trained diffusion-based text-to-speech (TTS) model using minimal untranscribed data. To fine-tune the feed-forward diffusion-based model with untranscribed data, we use the self-supervised unit representation as a pseudo transcript and plug the unit encoder into the pre-trained TTS model. We first train the unit encoder to provide speech content to the fixed diffusion decoder and then fine-tune only the diffusion decoder for speaker adaptation using <unit, speech> pair. Leveraging our model adapted to target speakers, we perform tasks such as TTS and voice conversion (VC) in a personalized manner. Compared to the previous baseline models, UnitSpeech achieves comparable results on personalized TTS and voice conversion tasks for various speakers. Our model also shows widespread adaptive performance on real-world data and other tasks that input text or unit.
...
- Existing speech-to-unit models use [unit-HiFi-GAN](https://huggingface.co/facebook/unit_hifigan_mhubert_vp_en_es_fr_it3_400k_layer11_km1000_lj_dur) trained on single speaker as speech synthesis module when generating the target language's speech.
- Replacing the unit-HiFi-GAN with UnitSpeech and combining it with [pre-trained speech-to-unit model](https://huggingface.co/facebook/xm_transformer_s2ut_800m-es-en-st-asr-bt_h1_2022), we show the possibility of personalization of speech-to-speech translation.
